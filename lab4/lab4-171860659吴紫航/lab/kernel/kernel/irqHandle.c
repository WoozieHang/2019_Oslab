...
void syscallWriteStdOut(struct StackFrame *sf) {
	...
	for (i = 0; i < size; i++) {
		...
		//oslab4 output answer for checking result
		putChar(character);
...
void syscallSemWait(struct StackFrame *sf) {
	//disableInterrupt();
	uint32_t dec_index=(uint32_t)sf->edx;
	if(dec_index<MAX_SEM_NUM&&sem[dec_index].state==1){
		sem[dec_index].value-=1;
		if(sem[dec_index].value<0){
			pcb[current].state=STATE_BLOCKED;
			struct ListHead* index=&(sem[dec_index].pcb);
			while(index->next!=&(sem[dec_index].pcb))
				index=index->next;

			pcb[current].blocked.next=&(sem[dec_index].pcb);
			sem[dec_index].pcb.prev=&(pcb[current].blocked);

			pcb[current].blocked.prev=index;
			index->next=&(pcb[current].blocked);
...
void syscallSemPost(struct StackFrame *sf) {
	//disableInterrupt();
	uint32_t inc_index=(uint32_t)sf->edx;
	if(inc_index<MAX_SEM_NUM&&sem[inc_index].state==1){
		
		if(sem[inc_index].value<0){
			sem[inc_index].value+=1;
			struct ListHead* awake_index=sem[inc_index].pcb.next;
			sem[inc_index].pcb.next=awake_index->next;
			awake_index->next->prev=awake_index->prev;
			uint32_t pid_index;
			pid_index=(uint32_t)&(pcb[current].pid)-(uint32_t)&(pcb[current].blocked)+(uint32_t)awake_index;
			uint32_t id=*(uint32_t*)pid_index;
			pcb[id].state=STATE_RUNNABLE;
		}
		else sem[inc_index].value+=1;
		pcb[current].regs.eax =0;
	}
	else
		pcb[current].regs.eax =-1;
...
void syscallSemDestroy(struct StackFrame *sf) {
	uint32_t destroy_index=(uint32_t)sf->edx;
	if(destroy_index<MAX_SEM_NUM&&sem[destroy_index].state==1){
		sem[destroy_index].state=0;
		pcb[current].regs.eax =0;
	}
	else pcb[current].regs.eax =-1;
	return;
